...
public class GuestNetwork {
    ...
    private const int HOST_UNREACHABLE_TIMEOUT = 5;     // In seconds
    ...
    public void ConnectToHost(string hostIP) {
        ...
        new Thread(() => {
            try {
                ...
                Debug.Log("Connected to Host: "+ep.Address);
        ...
        new Thread(() => {
            ...
            while (reconAttempts < MAX_RECONNECT_ATTEMPTS && !socketReady) {
                ...
                try {
                    ...
                    if (guestName != "")
                        Broadcast("new/" + guestName + "/" + IPv4() + "/" + isHider);

                    Debug.Log("Connected to Host(" + reconAttempts + "): " + ep.Address);
                } catch (Exception e) {
                    Debug.Log("Socket error(" + reconAttempts + "): " + e.Message);
                }
            }
            if (!socketReady) {
